...
import json
import numpy as np
from pytorch_ares.dataset_torch.cifar_dataset import cifar10_dist
...
def combine_tensors(target_tensor, device, world_size, batch_num):
    out_gather=[torch.ones_like(target_tensor).to(device) for _ in range(world_size)]
    dist.all_gather(out_gather, target_tensor, async_op=False)
    out_gather=torch.cat(out_gather)
...
if __name__ == '__main__':
    ...
    #dataset 
    parser.add_argument('--batchsize', default=50, help= 'batchsize for this model')
    ...
    # arguments for distributed attack
    ...
    test_dataset = cifar10_dist(args.batchsize, args.cifar10_path, need_dataset=True)
    # dist dataloader
    train_sampler = torch.utils.data.distributed.DistributedSampler(test_dataset,shuffle=False)
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batchsize, sampler=train_sampler)
    ...
    with open(os.path.join(args.result_path, "{}.csv".format("result")), "w") as f:
        ...
        for model_path in models:
            ...
            for i, (image,labels) in enumerate(test_loader, 1):
                ...
                labels_gather=combine_tensors(target_tensor=labels, device=device, world_size=world_size, batch_num=args.batchsize)
                out_gather=combine_tensors(target_tensor=out, device=device, world_size=world_size, batch_num=args.batchsize)

                test_num += (out_gather == labels_gather).sum()

                # ind_to_fool = acc.nonzero().squeeze()
                # if len(ind_to_fool.shape) == 0: ind_to_fool = ind_to_fool.unsqueeze(0)

                # acc_gather=(out_gather == labels_gather)
                # ind_to_fool_num = acc_gather.nonzero().squeeze()

                # if ind_to_fool_num.numel() != 0:
                x_to_fool, y_to_fool = image, labels
                # false_class += len(y_to_fool)
                # y_to_fool_gather=combine_tensors(target_tensor=y_to_fool, device=device, world_size=world_size, batch_num=args.batchsize)
                y_to_fool_gather=labels_gather
                # for temp in y_to_fool_gather:
                #     false_class += len(temp)
                ...
                out_adv_pgd_gather=combine_tensors(target_tensor=out_adv_pgd, device=device, world_size=world_size, batch_num=args.batchsize)
                out_adv_autoattack_gather=combine_tensors(target_tensor=out_adv_autoattack, device=device, world_size=world_size, batch_num=args.batchsize)

                test_num_pgd += (out_adv_pgd_gather == y_to_fool_gather).sum()
                test_num_aa += (out_adv_autoattack_gather == y_to_fool_gather).sum()

                # out_adv_pgd = torch.argmax(out_adv_pgd, dim=1)
                acc_pgd = (out_adv_pgd_gather != y_to_fool_gather)
                
                # out_adv_autoattack = torch.argmax(out_adv_autoattack, dim=1)
                acc_aa = (out_adv_autoattack_gather != y_to_fool_gather)
                
                robust_num += (acc_pgd | acc_aa).nonzero().numel()
                torch.cuda.synchronize()
                ...
                if i % 1 == 0:
                    num = i*batchsize*world_size
                    test_acc = test_num.item() / num
                    print("epoch: %d %s clean_acc: %.2f %%" %(i, net_without_ddp.name, test_acc*100 ))
                    
                    # if false_class!=0:
                    test_acc_pgd = test_num_pgd.item() / num
                    test_acc_aa = test_num_aa.item() / num
                    
                    print("epoch: %d %s pgd_acc: %.2f %%" %(i, net_without_ddp.name, test_acc_pgd*100))
                    print("epoch: %d %s autoattack_acc: %.2f %%\n" %(i, net_without_ddp.name, test_acc_aa*100))
            
            total_num = len(test_loader.dataset)
            print("total_num:%d" %(total_num))
            print("test_num:%d" %(test_num.item()))
            ...
            print("%s clean_acc: %.2f %%" %(net_without_ddp.name, round(final_test_acc*100,2)))
            print("%s pgd_acc: %.2f %%" %(net_without_ddp.name, round(success_num_pgd*100,2)))
            print("%s autoattack_acc: %.2f %%\n" %(net_without_ddp.name, round(success_num_aa*100,2)))
            print("%s robust_acc: %.2f %%\n" %(net_without_ddp.name, round(robust_acc*100,2)))
           
            f.write("{},{},{}\n".format(name, net_without_ddp.name, round(final_test_acc*100,2)))
            f.write("{},{},{}\n".format(pgd_name, net_without_ddp.name, round(success_num_pgd*100,2)))
            f.write("{},{},{}\n".format(aa_name, net_without_ddp.name, round(success_num_aa*100,2)))
            f.write("{},{},{}\n".format(robust, net_without_ddp.name, round(robust_acc*100,2)))
