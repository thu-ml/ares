...
model_path = os.path.join(path,'model/ciafar10_checkpoints/cifar10wrn_baseline_epoch_4.pt')


def load(device):
    model = Pre_Training(device)
    model.load()
    model.name = 'pre_training'
    return model
...
class Pre_Training(torch.nn.Module):
    ...
    def forward(self, x):
        labels = self.model(2.0*x.to(self.device)-1.0)
        return labels
