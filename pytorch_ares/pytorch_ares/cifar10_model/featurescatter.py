...
model_path = os.path.join(path,'model/ciafar10_checkpoints/checkpoint-199-ipot')


def load(device):
    model = Feature_Scatter(device)
    model.load()
    model.name = 'feature_scatter'
    return model
...
class Feature_Scatter(torch.nn.Module):
    ...
    def forward(self, x):
        labels = self.model(2.0*x.to(self.device)-1.0)[0]
        return labels
...
if __name__ == '__main__':
    if not os.path.exists(model_path):
        ...
        url = 'https://drive.google.com/file/d/1FXgE7llvQoypf7iCGR680EKQf9cARTSg/view'
