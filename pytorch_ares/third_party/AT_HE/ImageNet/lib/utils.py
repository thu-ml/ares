...
def parse_config_file(args):
    ...
    # Add the output path
    config.output_name = config.TRAIN.arch + '_' + '{:s}_step{:d}_eps{:d}_repeat{:d}'.format(args.output_prefix,
                         int(config.ADV.fgsm_step), int(config.ADV.clip_eps), 
                         config.ADV.n_repeats) + '_bs' + str(config.DATA.batch_size) + FNname + WNname + '_s' + str(config.s_HE) + '_margin' + str(config.angular_margin_HE)
    return config
