...
def test(args):
    ...
    attack = AutoAttack(net,norm=args.norm,steps=args.steps, query=args.n_queries, eps=args.eps, version=args.version,device=device)
    ...
    if args.dataset_name == "imagenet":
        for i, (image,labels,target_labels) in enumerate(test_loader, 1):
            ...
            adv_image= attack.run_standard_evaluation(image, labels,batchsize)
