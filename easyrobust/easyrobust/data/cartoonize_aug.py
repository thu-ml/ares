...
class CartoonAugmentor:
    def __init__(self):
        self.model_sty1 = Transformer()
        self.model_sty2 = Transformer()
        self.model_sty3 = Transformer()
        self.model_sty4 = Transformer()

        self.model_sty1.load_state_dict(model_zoo.load_url('http://alisec-competition.oss-cn-shanghai.aliyuncs.com/xiaofeng/easy_robust/pretrained_models/Hayao_net_G_float.pth'))
        self.model_sty2.load_state_dict(model_zoo.load_url('http://alisec-competition.oss-cn-shanghai.aliyuncs.com/xiaofeng/easy_robust/pretrained_models/Hosoda_net_G_float.pth'))
        self.model_sty3.load_state_dict(model_zoo.load_url('http://alisec-competition.oss-cn-shanghai.aliyuncs.com/xiaofeng/easy_robust/pretrained_models/Paprika_net_G_float.pth'))
        self.model_sty4.load_state_dict(model_zoo.load_url('http://alisec-competition.oss-cn-shanghai.aliyuncs.com/xiaofeng/easy_robust/pretrained_models/Shinkai_net_G_float.pth'))

        self.model_sty1.eval()
        self.model_sty2.eval()
        self.model_sty3.eval()
        self.model_sty4.eval()

        if torch.cuda.is_available():
            self.model_sty1.cuda()
            self.model_sty2.cuda()
            self.model_sty3.cuda()
            self.model_sty4.cuda()

    def __call__(self, image, style_type=0):
        
        image = image[:, [2, 1, 0], :, :]
        if style_type == 0:
            image = self.model_sty1(image)
        elif style_type == 1:
            image = self.model_sty2(image)
        elif style_type == 2:
            image = self.model_sty3(image)
        elif style_type == 3:
            image = self.model_sty4(image)
        image = image[:, [2, 1, 0], :, :]
