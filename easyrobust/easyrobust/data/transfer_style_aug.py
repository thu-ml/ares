...
Shape-Texture Debiased Neural Network Training (ICLR2021)
Paper link: https://arxiv.org/abs/2010.05981
...
class StyleTransfer:
    def __init__(self, mean=None, std=None, decoder_ckpt='http://alisec-competition.oss-cn-shanghai.aliyuncs.com/xiaofeng/easy_robust/pretrained_models/decoder.pth', vgg_ckpt='http://alisec-competition.oss-cn-shanghai.aliyuncs.com/xiaofeng/easy_robust/pretrained_models/vgg_normalised.pth'):
        self.decoder = decoder
        self.decoder.eval()
        self.decoder.load_state_dict(model_zoo.load_url(decoder_ckpt))
        self.decoder.to('cuda')

        self.vgg = vgg
        self.vgg.eval()
        self.vgg.load_state_dict(model_zoo.load_url(vgg_ckpt))
    ...
    def __call__(self, image, label, alpha, replace=True, label_mix_alpha=0):
        ...
        # normalize
        if self.mean is not None and self.std is not None:
            stylized_image = (stylized_image - self.mean[:, None, None]) / self.std[:, None, None]
            image = (image - self.mean[:, None, None]) / self.std[:, None, None]

        if replace:
            return stylized_image, (label, label_style, torch.ones(n).to('cuda') * label_mix_alpha)
        else:
            label1 = torch.cat([label, label])
            label2 = torch.cat([label_style, label_style])
            label_weight = torch.cat([torch.zeros(n), torch.ones(n) * label_mix_alpha]).to('cuda')
